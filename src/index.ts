...
async function setupBot(telegraf: Telegraf) {
    /*
     * Here you can define how the Telegraf instance will react to user's messages.
     */
    telegraf.on("text", context => {
        context.reply(`Received message: ${context.message.text}`);
    });
}

async function launchBot(): Promise<boolean> {
...
async function start() {
    ...
    const launched = await launchBot();
